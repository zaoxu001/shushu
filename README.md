...
tianzhi-core 是[天秩](https://tianzhi.live)的计算核心。天秩是东方术数的研究与学习平台，涵盖八字、六壬、六爻、奇门、紫微、梅花。
...
tianzhi_core/
  ...
  liuren/      六壬    月将加时 → 天地盘 → 四课 → 九宗门三传 → 天将
  ziwei/       紫微    安命身 → 五行局 → 安星 → 四化 → 限 → 格局
  ...                  其余各门依同样的次第展开
```
...
| 层 | 模块 | 提供 |
|---|---|---|
| 起课 | `liuren.pan` | 月将（中气换将）、天地盘、四课、九宗门三传、十二天将、旬空、遁干、六亲、神煞 |
| 课体 | `liuren.keti` `bifa` | 课经诸课体、毕法赋中可在课式上判定的句子 |
| 占事 | `liuren.zhanshi` | 按所问之事（事业、求财、感情、出行、寻物）取类神，逐条推出此课对此事的论断、三传始终、急迟与应期 |

取传依《六壬大全》（四库全书本）。课经卷五至卷八的起例课 48 个逐一核对三传，全合，留作回归测试。
涉害一门诸家不一：《六壬大全》归本家数克、取深者，《大六壬指南》只取孟仲季、不数深浅；本包从《大全》，
数克时临位不计、寄宫干计入，与书中七处实例一致。

占事照书中断法：类神依卷八《事類神》，「入課傳為取用，皆入皆不入責將神」；求财、占婚、失物各依卷二该将之下的专条；
三传始终、用生终死、母子顺逆、事之急迟、应期应日依卷三「發用」。卷三诸句注中的实例（急迟两例、应期两例、应日两例、
生死四例、母子两例）都留作测试。各条的方向（顺还是阻）照原文，合成总倾向时的加法是本包取值。

## 紫微模块

| 层 | 模块 | 提供 |
|---|---|---|
| 排盘 | `ziwei.chart` | 命宫身宫、十二宫与宫干、五行局、十四主星、六吉六煞、禄存天马、生年四化、杂曜、截路旬中空亡、长生与博士十二神、命主身主、大限小限、庙旺利陷 |
| 运限 | `ziwei.yunxian` | 大限（童限）、小限、流年、流月（斗君）、流日、流时：各层命宫、宫名、四化与流曜；流年岁前十二神与天德月德 |
| 断语 | `ziwei.duanyu` | 卷三「论诸星同垣」与卷一骨髓赋、女命骨髓赋、太微赋、增补太微赋、形性赋，六百四十八句逐句写成判定条件；读得准的四百三十四句判，其余不判并写明缘故 |
| 运限论断 | `ziwei.xianyun` | 卷三论大限十年祸福、行限分南北斗、羊陀迭并、七杀重逢、限遇十二支所忌、太岁小限遇十二宫吉凶 |
| 格局 | `ziwei.geju` | 卷一「定富局」「定贵局」「定贫贱局」中释义可在盘上判定的条目，每条给出原句与本盘所据 |

安星依《紫微斗数全书》卷二「安身命例」诸诀，每颗星注明出自哪一诀；五张定紫微图逐日核对（录文两处错格已对明刻本确认）。
庙旺取卷二命宫一节各星「某宫入庙 / 陷地」之句，书中没写的格留空，不从他书补。
与开源实现 [iztro](https://github.com/SylarLong/iztro) 对拍数千盘：分歧只在三处有书可据的取舍，传同样参数后逐星一致——
闰月作下月（书：「若闰月正月生者要在二月内起安身命」）、壬年天府化科（「壬梁紫府武」）、长生男顺女逆（「男命顺数、女命逆数」）；
另晚子时逢月末，本包按次日农历跨入下月。格局释义有六条按安星法字面永不成立（如「武梁来夹」，武曲与天梁永不相隔一宫），
不判，列于 `ziwei.SKIPPED` 与 tianzhi-classics 的校勘记。

《全书》未载安法的杂曜（天官、天福、天厨、天巫、天月、阴煞、孤辰寡宿、蜚廉、破碎、华盖咸池、天才天寿、恩光天贵）
与岁前、将前十二神，照后世通行排法补上，出处标「通行排法」，`tongxing=False` 可关；与 iztro 对拍两千盘一致。
运限与 iztro 对拍一千五百组（生辰 × 任意时刻）六层全合；流月干支按农历月五虎遁，与流年同以正月初一为界。
每宫另给宫干飞化（化入哪一宫、是否自化）。

庙旺两卷并用：卷二命宫一节逐宫所言与卷三「诸星同垣」节首所列之表有十四格出入（见校勘记），默认取卷三、以卷二补其未载，
`miaowang="juan2"` 可切。断语的判定条件是本包据原文与小注拟定的读法，每条带 reading 说明；
按安星法不可能出现的组合、所指不明的术语、须放宽原文才能判的句子一律不判。
...
流派分歧之处做成参数：晚子时归哪一天、阴干有没有刃、取用以格局为先还是调候为先、紫微闰月与壬年化科，默认值另附理由。
