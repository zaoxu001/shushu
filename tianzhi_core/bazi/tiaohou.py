...
校勘（2026-09）
---------------
调候表一百二十格已逐格回《穷通宝鉴》原文核对（据维基文库本），改正五处。
逐格的原文、今译与判归见 https://github.com/zaoxu001/tianzhi-classics
的 bazi/entries/qiongtong.json。

- 甲·戌 「甲庚」→「丁庚」。讹误：「甲」为日干自身，不能为自身之调候用神。
  非流派之异，疑涉该段「譬如木为犁，能疏季土」中「木为犁」之喻。依原文
  「独爱丁火」取丁，依「癸庚决不可少也」取庚。
- 甲·午 「癸丁」→「癸丁庚」。脱漏：原文「五月先癸后丁，庚金次之」明列三字。
- 甲·未 「癸庚丁」→「丁庚」。次序：原文「先丁后庚，无癸亦可」，癸不当居首。
- 甲·申 「庚丁壬」→「丁庚」。次序：原文「丁火为尊，庚金次之」，三秋总论亦
  云「先丁后庚」。壬在原文作「壬水无碍」，是不碍事而非取用。
- 乙·未 「癸丙」→「丙癸」。次序：原文「柱多金水，丙火为尊」。

次序并非无关紧要：climate_gods 的 primary 取首字，次序一改，取用即变。
其余一百一十五格与原文相合，其中五十格数据所多之字另见该月后文或该季总论，
不作改动。《金不换大运》与人元司令分日表尚未核对。
"""
